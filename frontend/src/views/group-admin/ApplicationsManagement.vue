  ...
  <div class="content-area">
    ...
    <div class="page-header">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="page-title">{{ $t('groupAdmin.applicationsManagement.title') }}</h1>
          <p class="page-subtitle">{{ $t('groupAdmin.applicationsManagement.subtitle') }}</p>
        </div>
        <button @click="openCreateModal" class="btn btn-primary" :disabled="appGroups.length === 0">
          <Icon icon="mdi:plus" class="h-4 w-4 mr-2" />
          {{ $t('groupAdmin.applicationsManagement.newApplication') }}
        </button>
      </div>
    </div>
    ...
    <div v-if="appGroups.length === 0" class="mb-6 p-4 bg-yellow-900/20 border border-yellow-600 rounded-lg">
      <div class="flex items-start">
        ...
        <div>
          <h3 class="text-yellow-500 font-medium">{{ $t('groupAdmin.applicationsManagement.noAppGroups') }}</h3>
          <p class="text-sm text-yellow-200 mt-1">
            {{ $t('groupAdmin.applicationsManagement.noAppGroupsHelp') }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="filters-container">
      <div class="search-container flex-1">
        ...
        <input
          v-model="searchQuery"
          type="text"
          :placeholder="$t('groupAdmin.applicationsManagement.searchPlaceholder')"
          class="form-input search-input"
        />
      </div>
      <select v-model="groupFilter" class="form-select w-full sm:w-auto">
        <option value="">{{ $t('groupAdmin.applicationsManagement.groupFilter.all') }}</option>
      ...
      <select v-model="statusFilter" class="form-select w-full sm:w-auto">
        <option value="">{{ $t('groupAdmin.applicationsManagement.status.all') }}</option>
        <option value="active">{{ $t('groupAdmin.applicationsManagement.status.active') }}</option>
        <option value="inactive">{{ $t('groupAdmin.applicationsManagement.status.inactive') }}</option>
      </select>
    </div>
      ...
      >
        <div class="card-header">
          ...
          <div class="flex gap-2">
            <button @click="editApplication(app)" class="btn-ghost" :title="$t('groupAdmin.applicationsManagement.actions.edit')">
              <Icon icon="mdi:pencil" class="h-4 w-4" />
            </button>
            <button @click="confirmDelete(app)" class="btn-ghost text-red-400 hover:text-red-300" :title="$t('groupAdmin.applicationsManagement.actions.delete')">
        ...
        <div class="card-content">
          ...
          <div class="flex items-center justify-between text-sm">
            <span :class="app.is_active ? 'badge badge-success' : 'badge badge-secondary'">
              {{ app.is_active ? $t('groupAdmin.applicationsManagement.statusBadge.active') : $t('groupAdmin.applicationsManagement.statusBadge.inactive') }}
            </span>
            <span class="text-gray-500">{{ $t('groupAdmin.applicationsManagement.order') }} {{ app.order || 0 }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="applications.length === 0" class="empty-state">
      ...
      <h3 class="empty-state-title">{{ $t('groupAdmin.applicationsManagement.empty.title') }}</h3>
      <p class="empty-state-description">{{ $t('groupAdmin.applicationsManagement.empty.description') }}</p>
      <button
      ...
      >
        <Icon icon="mdi:plus" class="h-4 w-4 mr-2" />
        {{ $t('groupAdmin.applicationsManagement.empty.createButton') }}
      </button>
    </div>

    <div v-else class="empty-state">
      <Icon icon="mdi:magnify" class="empty-state-icon" />
      <h3 class="empty-state-title">{{ $t('groupAdmin.applicationsManagement.noResults.title') }}</h3>
      <p class="empty-state-description">{{ $t('groupAdmin.applicationsManagement.noResults.description') }}</p>
    </div>
        ...
        <div class="modal-panel sm:max-w-lg sm:w-full">
          <div class="modal-header">
            <div class="flex items-start gap-3">
              ...
              <div>
                <h3 class="modal-title">{{ $t('groupAdmin.applicationsManagement.deleteConfirm.title') }}</h3>
                <p class="modal-subtitle">
                  {{ $t('groupAdmin.applicationsManagement.deleteConfirm.message', { name: applicationToDelete?.name }) }}
                </p>
              </div>
            </div>
          </div>
...
const getAppGroupName = (appGroupId) => {
  ...
  return group ? group.name : t('common.noGroup')
}
...
const deleteApplication = async () => {
  ...
  try {
    ...
    appStore.showError($t('groupAdmin.applicationsManagement.deleteError'))
  } finally {
