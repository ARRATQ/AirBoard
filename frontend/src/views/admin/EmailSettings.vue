          ...
          <div class="card-header flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              <label class="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  v-model="smtpConfig.is_enabled"
                  class="sr-only peer"
                />
                <div
                  class="w-11 h-6 bg-gray-600 dark:bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"
                ></div>
              </label>
            </div>
          </div>
          ...
          <div class="p-6 space-y-6">
              ...
              <div class="form-group">
                ...
                <div class="relative">
                  <input
                    ...
                    :placeholder="
                      smtpConfig.id
                        ? $t('email.passwordKeep')
                        : $t('email.passwordPlaceholder')
                    "
                  />
                  <button
                    type="button"
            ...
            <div class="flex flex-wrap gap-6">
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  v-model="smtpConfig.use_starttls"
                  class="form-checkbox"
                />
                <span class="text-sm text-gray-300 dark:text-gray-400">{{
                  $t("email.useSTARTTLS")
            ...
            <!-- Last Test Info -->
            <div
              v-if="smtpConfig.last_tested_at"
              class="p-3 rounded-lg"
              :class="
                smtpConfig.last_test_success
            ...
            >
              <div class="flex items-center gap-2">
                ...
                >
                  {{
                    smtpConfig.last_test_success
                      ? $t("email.lastTestSuccess")
                      : $t("email.lastTestFailed")
                  }}
          ...
          <div class="p-6">
            ...
            <div v-if="selectedTemplate" class="space-y-6">
              ...
              >
                <div class="flex items-center gap-3">
                  <Icon
                    :icon="getTemplateIcon(selectedTemplate.type)"
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    v-model="selectedTemplate.is_enabled"
                    class="sr-only peer"
                  />
                  <div
                    class="w-11 h-6 bg-gray-600 dark:bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500"
              ...
              <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="variable in getVariablesForType(
              ...
              <div class="form-group">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    class="text-sm text-blue-400 dark:text-blue-300 hover:text-blue-300 dark:hover:text-blue-200 flex items-center gap-1"
                  >
    ...
    <div v-show="activeTab === 'logs'">
      ...
      <div class="card">
        <div class="card-header flex items-center justify-between">
          ...
          <button
            @click="loadLogs"
            class="btn btn-ghost btn-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
          >
            ...
            <table class="min-w-full">
              <thead>
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  <th
                    class="text-left py-3 px-4 text-sm font-medium text-gray-400 dark:text-gray-500"
                  >
                    {{ $t("email.logType") }}
                  </th>
                  <th
                  ...
                  <th
                    class="text-center py-3 px-4 text-sm font-medium text-gray-400 dark:text-gray-500"
                  >
                    {{ $t("email.logRecipients") }}
                  </th>
                  <th
                    class="text-center py-3 px-4 text-sm font-medium text-gray-400 dark:text-gray-500"
                  >
                ...
                >
                  <td class="py-3 px-4">
                    <span class="flex items-center gap-2">
                      <Icon
                        :icon="getTemplateIcon(log.template_type)"
                        class="h-4 w-4 text-gray-400 dark:text-gray-500"
                      />
                      <span class="text-sm text-gray-300 dark:text-gray-400">{{
                        getTemplateName(log.template_type)
                      }}</span>
                    </span>
                  </td>
        ...
        <div class="modal-panel">
          <div class="modal-header">
            ...
            <button
              ...
              class="text-gray-400 dark:text-gray-500 hover:text-white dark:hover:text-gray-300"
            >
...
const saveTemplate = async () => {
  ...
  try {
    const result = await emailService.updateTemplate(
    ...
    appStore.showError(
      error.response?.data?.error || t("email.templateSaveError"),
    );
  } finally {
...
const resetTemplate = async () => {
  ...
  try {
    const result = await emailService.resetTemplate(
      selectedTemplate.value.type,
    );
    const index = templates.value.findIndex((t) => t.type === result.type);
    if (index !== -1) {
    ...
    appStore.showError(
      error.response?.data?.error || t("email.templateResetError"),
    );
  }
};
...
const getStatusClass = (status) => {
  const classes = {
    ...
    sending:
      "bg-yellow-100 dark:bg-yellow-800/50 text-yellow-800 dark:text-yellow-200 border border-yellow-200 dark:border-yellow-700",
    failed:
      "bg-red-100 dark:bg-red-800/50 text-red-800 dark:text-red-200 border border-red-200 dark:border-red-700",
    pending:
      "bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700",
  };
