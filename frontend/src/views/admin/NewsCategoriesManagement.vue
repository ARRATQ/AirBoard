  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
    <div class="max-w-5xl mx-auto space-y-6">
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ t('admin.newsCategories') }}</h1>
          <p class="text-gray-600 dark:text-gray-400 mt-1">{{ t('admin.newsCategoriesHelp') }}</p>
        </div>
      ...
      <div class="card p-5 border border-gray-200 dark:border-gray-700">
        ...
        <div v-else-if="categories.length === 0" class="empty-state py-8">
          <Icon icon="mdi:tag-multiple-outline" class="text-4xl text-gray-400" />
          <p>{{ t('admin.noCategory') }}</p>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-center gap-3 flex-1">
              <Icon :icon="category.icon || 'mdi:tag'" class="text-2xl" :style="{ color: category.color }" />
              <div class="flex-1">
                <p class="font-semibold text-gray-900 dark:text-white">{{ category.name }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  slug: <code class="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{{ category.slug }}</code>
                  · {{ t('common.order') }}: {{ category.order }}
                </p>
                <p v-if="category.description" class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ category.description }}</p>
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div class="flex gap-2">
                    <input v-model="form.icon" class="form-input flex-1" placeholder="mdi:tag" />
                    <div class="flex items-center justify-center px-4 bg-gray-100 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600">
                      <Icon :icon="form.icon || 'mdi:tag'" class="text-2xl" :style="{ color: form.color }" />
                    </div>
                  </div>
                </div>
...
const loadCategories = async () => {
  try {
    ...
    if (window.$toast) {
      window.$toast.error(error.response?.data?.error || t('admin.categoryLoadError'))
    }
  } finally {
    loading.value = false
  }
...
const saveCategory = async () => {
  const payload = {
    ...
    icon: form.value.icon.trim() || 'mdi:tag',
  ...
  try {
    ...
    if (window.$toast) {
      window.$toast.error(error.response?.data?.error || t('admin.categorySaveError'))
    }
  }
}
...
const removeCategory = async (category) => {
  ...
  try {
    ...
    if (window.$toast) {
      const message = error.response?.data?.error || t('admin.categoryDeleteError')
