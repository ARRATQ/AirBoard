      ...
      <div class="modal-panel sm:max-w-2xl sm:w-full">
        <form @submit.prevent="handleSubmit">
            ...
            <div class="flex items-center justify-between">
              <div>
                <h3 class="modal-title">
                  {{ isEdit ? $t('groupAdmin.applicationModal.title.edit') : $t('groupAdmin.applicationModal.title.new') }}
                </h3>
                <p class="modal-subtitle">
                  {{ isEdit ? $t('groupAdmin.applicationModal.subtitle.edit') : $t('groupAdmin.applicationModal.subtitle.new') }}
                </p>
              </div>
              <button
                type="button"
          ...
          <div class="modal-content space-y-6">
            ...
            <div>
              <div class="section-header">
                ...
                <h4 class="section-title">{{ $t('groupAdmin.applicationModal.basicInfo') }}</h4>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-group">
                  <label for="name" class="form-label form-label-required">
                    {{ $t('groupAdmin.applicationModal.applicationName') }}
                  </label>
                  <input
                    id="name"
                    v-model="form.name"
                    ...
                    required
                    class="form-input"
                    placeholder="{{ $t('groupAdmin.applicationModal.applicationName') }}"
                  />
                ...
                <div class="form-group">
                  <label for="app_group_id" class="form-label form-label-required">
                    {{ $t('groupAdmin.applicationModal.appGroup') }}
                  </label>
                  <select
                    id="app_group_id"
                    ...
                    class="form-select"
                  >
                    <option value="">{{ $t('groupAdmin.applicationModal.selectGroup') }}</option>
                    <option
                  ...
                  <p v-if="availableAppGroups.length === 0" class="form-help text-yellow-500">
                    {{ $t('groupAdmin.applicationModal.noPrivateGroups') }}
                  </p>
                </div>
              </div>
              ...
              <div class="form-group">
                <label for="url" class="form-label form-label-required">
                  {{ $t('groupAdmin.applicationModal.applicationUrl') }}
                </label>
                <input
                  id="url"
                  v-model="form.url"
                  ...
                  required
                  class="form-input"
                  :placeholder="$t('groupAdmin.applicationModal.urlPlaceholder')"
                />
                <p v-if="errors.url" class="form-error">{{ errors.url }}</p>
                <p class="form-help">{{ $t('groupAdmin.applicationModal.urlHelp') }}</p>
              </div>

              <div class="form-group">
                <label for="description" class="form-label">
                  {{ $t('groupAdmin.applicationModal.description') }}
                </label>
                <textarea
                  id="description"
                  ...
                  rows="2"
                  class="form-textarea"
                  :placeholder="$t('groupAdmin.applicationModal.descriptionPlaceholder')"
                ></textarea>
              </div>
            </div>
            ...
            <div>
              <div class="section-header">
                ...
                <h4 class="section-title">{{ $t('groupAdmin.applicationModal.appearance') }}</h4>
              </div>

              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Preview -->
                <div class="form-group">
                  <label class="form-label">{{ $t('groupAdmin.applicationModal.preview') }}</label>
                  <div class="card p-4">
                    <div class="flex items-center space-x-4">
                      ...
                      <div class="flex-1">
                        <div class="font-medium text-white">{{ form.name || $t('groupAdmin.applicationModal.previewName') }}</div>
                        <div class="text-sm text-gray-400">{{ form.url || $t('groupAdmin.applicationModal.previewUrl') }}</div>
                      </div>
                ...
                <div class="space-y-4">
                  <div class="form-group">
                    <label for="color" class="form-label">{{ $t('groupAdmin.applicationModal.color') }}</label>
                  ...
                  <IconInput
                    v-model="form.icon"
                    :label="$t('groupAdmin.applicationModal.icon')"
                  ...
                  <div class="form-group">
                    <label for="order" class="form-label">{{ $t('groupAdmin.applicationModal.displayOrder') }}</label>
                    <input
                      id="order"
            ...
            <div>
              <div class="section-header">
                ...
                <h4 class="section-title">{{ $t('groupAdmin.applicationModal.behaviorOptions') }}</h4>
              </div>
              ...
              <div class="space-y-3">
                <div class="card p-4">
                  <label class="flex items-center space-x-3 cursor-pointer">
                    ...
                    <span class="text-sm text-gray-300">
                      {{ $t('groupAdmin.applicationModal.openInNewTab') }}
                    </span>
                  </label>
                </div>
                ...
                <div class="card p-4">
                  <label class="flex items-center space-x-3 cursor-pointer">
                    ...
                    />
                    <span class="text-sm text-gray-300">
                      {{ $t('groupAdmin.applicationModal.activeAndVisible') }}
                    </span>
                  </label>
                </div>
              </div>
          ...
          <div class="modal-footer">
            ...
            >
              {{ $t('groupAdmin.applicationModal.cancel') }}
            </button>
            <button
              type="submit"
            ...
            >
              ...
              {{ isEdit ? $t('groupAdmin.applicationModal.update') : $t('groupAdmin.applicationModal.create') }}
            </button>
          </div>
        </form>
      </div>
...
const validateForm = () => {
  ...
  if (!form.name.trim()) {
    errors.value.name = $t('groupAdmin.applicationModal.errors.nameRequired')
  }

  if (!form.url.trim()) {
    errors.value.url = $t('groupAdmin.applicationModal.errors.urlRequired')
  } else if (!isValidUrl(form.url)) {
    errors.value.url = $t('groupAdmin.applicationModal.errors.invalidUrl')
  }

  if (!form.app_group_id) {
    errors.value.app_group_id = $t('groupAdmin.applicationModal.errors.appGroupRequired')
  }
